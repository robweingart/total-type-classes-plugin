...
import GHC (Class, HsMatchContext (FunRhs, mc_fun), instanceDFunId)
...
import GHC.Tc.Utils.Monad (setGblEnv, updTopFlags, updGblEnv, addErrTc, failWithTc, setCtLocM, tryTc, mapMaybeM, mapAndUnzipM)
...
mk_check_result_inst :: Type -> TyCon -> (Bool, Bool, Bool) -> TcPluginM EvTerm
mk_check_result_inst ck tc (ex_res, term_res, ctxt_res) = do
